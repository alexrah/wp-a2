...
class MP_Bounce_api extends MP_Db_connect
{
	...
	function __construct()
	{
		...
		$this->trace = new MP_Log($this->log_name, MP_ABSPATH, $this->class, false, $this->log_option_name);
...
// process
	...
	function process_message()
	{
		...
		else
		{
			...
			$usermeta = MP_Usermeta::get($mp_user_id, $this->metakey);
			if (!$usermeta)
			{
				$usermeta = array();
				$usermeta['bounce'] = 1;
				$usermeta['bounces'][$mail_id][] = $bounce;	
				MP_Usermeta::add($mp_user_id, $this->metakey, $usermeta);
			}
			else
			{
				...
				if (!MP_Usermeta::add(    $mp_user_id, $this->metakey, $usermeta, true))
					MP_Usermeta::update($mp_user_id, $this->metakey, $usermeta);
			}
